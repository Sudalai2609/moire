...
function openLetterbox() {
  ...
  background:rgba(240,230,220,0.95);z-index:30;display:flex;flex-direction:column;
  align-items:center;justify-content:center;font-family:serif;color:#5a4a42;`;

  const list = getSaved().map(n => `<p>${n}</p>`).join('');
  panel.innerHTML = `<div style="max-width:80%">${list}</div>
  <textarea id="noteInput" placeholder="leave a note..." style="margin-top:20px;padding:10px;
  width:250px;height:80px;border-radius:8px;border:1px solid #c9a86a;font-family:serif;"></textarea>
  <button id="saveNote" style="margin-top:10px;padding:8px 20px;border-radius:8px;
  background:#c9a86a;border:none;color:white;">leave it</button>
