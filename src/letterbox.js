...
import { addLetter, listenLetters } from './firebase.js';
...
let currentLetters = [];
listenLetters(letters => { currentLetters = letters; });
...
function openLetterbox() {
  ...
  const list = currentLetters.map(n => '<p>' + n.text + '</p>').join('');
